    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card d-grid gap-3">
                <div class="card-header p-2" style="text-align:center;">إضافة معرف</div>
                ...
                <div class="card-body ">
                    ...
                    <form id="user_info_form" action="/saveleader" method="post">
                        @csrf
                        <!-- {{ csrf_field() }} -->
                        <input type="hidden" id="input_election_code" name="input_election_code" class="form-control" autocomplete="off" value="">
                        <!-- First Text Field -->
                        <div class="md-form">
                            <label for="dateField">اسم المرشد</label>
                            <input type="hidden" id="input_leader_code" name="input_leader_code" class="form-control">
                            <input type="text" id="input_leader" name="input_leader" class="form-control" oninput="fetchLeadersData(this)" autocomplete="off">
                            <div class="list-group" id="leader_chooser">

                            </div>
                            <label for="dateField">اسم العملية الانتخابية</label>
                            <input type="hidden" id="input_election_code" name="input_election" class="form-control">
                        ...
                        <div style="padding-top: 50px;align-items: center;">
                            ...
                            <button type="button" class="btn btn-danger btn-lg" onclick="clearForm('user_info_form')">مسح</button>
                        </div>
...
<script>
        ...
        .then(data => {
            ...
            for (let i = 0; i < result_data.length; i++) {
                $('#choose_users').append(
                    ...
                    result_data[i].group_code + '</th><th>' +
                    result_data[i].group_name + '</th></tr>'
                );
            }
    ...
    function fetchLeadersData(sender_var) {
        var inputValue = $(sender_var).val();

        // Your data to be sent
        const dataToSendleader = {
            user_name: inputValue,
        };

        // Convert data to URL parameters
        const params = new URLSearchParams(dataToSendleader).toString();
        ...
        fetch('/getUsers?'+params)
            ...
            .then(data => {
                ...
                var result_data = data;
                $('#leader_chooser').empty();
                for (let i = 0; i < result_data.length; i++) {
                    $('#leader_chooser').append(
                        '<a href="#" class="list-group-item list-group-item-action list-group-item-light"  id="' + result_data[i].profile_code + '" onclick="appendinputLeader(this)">' +
                        result_data[i].full_name + '</a>'
                    );
                }
    ...
    function appendinputLeader(sender_var) {
        var inputValue = $(sender_var).text();
        $('#input_leader').val(inputValue);
        $('#input_leader_code').val($(sender_var).attr('id'));
        $('#leader_chooser').empty();
    }
